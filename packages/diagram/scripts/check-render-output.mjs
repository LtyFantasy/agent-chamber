...
// 静态门：模板 <script> 块不得含阻塞式弹窗调用（alert/confirm/prompt/window.open）。
// 只扫 <script> 块内容——SVG 文本里的用户 IR 内容可能合法包含这些字样，不能误报
// （评审 architect#9/security#4）。断言失败时输出命中位置。
const scriptBlocks = [...html.matchAll(/<script\b[^>]*>([\s\S]*?)<\/script>/gi)];
const modalCalls = [];
scriptBlocks.forEach((block, blockIndex) => {
  const body = block[1] || '';
  for (const pattern of [/\balert\s*\(/g, /\bconfirm\s*\(/g, /\bprompt\s*\(/g, /\bwindow\.open\s*\(/g]) {
    let match;
    while ((match = pattern.exec(body)) !== null) {
      modalCalls.push(`script block #${blockIndex}: "${match[0]}" at offset ${match.index}`);
    }
  }
});
addCheck('no_modal_calls', modalCalls.length === 0, modalCalls);
