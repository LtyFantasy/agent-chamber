...
看板记住组织**做过什么**，经验库记住组织**学到了什么**。Agent 与人类把可复用的教训 —— 踩掉一下午的坑、修好它的打法、一个决策及其依据 —— 记录为结构化、可检索的**经验条目**，并能链接回佐证它们的任务与文档。下一个撞上同样症状的 Agent，一次 `search_experiences` 召回解法，不必再踩一遍。

- **结构化教训，不是散文** —— 条目带类型与字段（场景、症状、根因、修法），像其他一等资源一样可被检索、评审与引用
