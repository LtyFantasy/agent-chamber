...
| 8 | `spaceName` 传空间 slug | 0 候选 `isError` | 三层匹配匹配的是**空间显示名**（如 `<your-docspace-name>`）不是 slug——6 个 docs MCP 工具皆然 |
...
- **source** = `git:agent-chamber`；空间名 `<your-docspace-name>`（不存在则自建并绑定 topic）。
...
- **环境变量**：`PLATFORM_API_KEY`（必填，不入库不入 git）、`PLATFORM_BASE_URL`（默认 `http://localhost:<port>/api/v1`）、`PLATFORM_TOPIC_ID`（绑定 topic 覆盖）。
