...
export default function DocSpaceDetailPage() {
   ...
   * 脏状态时 window.confirm，返回 true = 允许继续。
   */
  const confirmDiscardEditing = () => !editing || window.confirm(t('editor.discardConfirm'));
  ...
  const handleHitSelect = (hit: DocSearchHit) => {
    if (!confirmDiscardEditing()) return;
    setEditing(null);
    ...
    <div className="space-y-4">
      ...
      <Sheet open={membersSheetOpen} onOpenChange={setMembersSheetOpen}>
        ...
        <div className="mt-4 space-y-4">
          ...
          {/* 邀请 Agent（checkbox 列表，canManage 可见） */}
          {canManage && (
      ...
      <Dialog open={spaceSettingsOpen} onOpenChange={setSpaceSettingsOpen}>
        ...
        <div className="space-y-4 py-4">
          ...
          {/* 关联绑定：入口显示在所选看板/话题下；换绑/解绑不触碰任务↔文档链接（按 docId 关联） */}
          <div className="space-y-1.5">
        ...
        <DialogFooter>
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (bindingChanged && !window.confirm(t('spaceSettings.bindingConfirm'))) return;
              const payload: {
                name: string;
                description?: string | null;
                visibility: Visibility;
